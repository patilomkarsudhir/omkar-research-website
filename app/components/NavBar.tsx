...
export default function NavBar() {
  ...
  <Link href="/" className="group flex items-center gap-6 md:gap-8">
          <span className="text-2xl md:text-3xl font-bold tracking-tight text-white/95 transition-colors group-hover:text-[var(--accent)]">{SITE.navName}</span>
    ...
    <nav className={`md:flex gap-6 text-sm ${open ? "flex flex-col absolute left-0 right-0 top-16 bg-[var(--panel)] p-4" : "hidden"} md:static md:p-0`}>
          {NAV_LINKS.map(link => (
            <Link
              key={link.href}
              href={link.href}
