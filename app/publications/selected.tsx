...
import selectedTitles from "../../data/selected.json";
import { loadPaperMapping, getPdfUrl, normalizeForMatching } from "./paper-utils";
...
const selected = selectedTitles as string[];

// Normalize titles for comparison
const normalizedSelected = selected.map(t => normalizeForMatching(t));
...
export default function SelectedPubs() {
    ...
    async function fetchData() {
      try {
        ...
        if (res.ok) {
          ...
          const allPubs: Pub[] = data.publications || [];
          
          // Filter to selected publications and maintain the order from selected.json
          const filteredPubs = normalizedSelected
            .map(normalizedTitle => 
              allPubs.find(p => normalizeForMatching(p.title) === normalizedTitle)
            )
            .filter((p): p is Pub => p !== undefined);
  ...
  return (
    <div className="grid gap-3">
      ...
      {!pubs.length && !loading && <div className="text-[var(--muted)] text-sm">No selected publications found.</div>}
    </div>
  );
}
