...
const TEMPLATES = [
    ...
    },
    {
        ...
        make: () => ({ name: 'Untitled', compiler: 'pdflatex', mainFile: 'main', files: [
                { id: 'main', name: 'main.tex', content: `\\documentclass[11pt]{article}
\\usepackage[utf8]{inputenc}
\\usepackage{amsmath, amssymb, amsthm}
\\usepackage{graphicx}
\\usepackage{hyperref}
...
` }
    ...
    },
    {
        ...
        make: () => ({ name: 'NeurIPS Submission', compiler: 'pdflatex', mainFile: 'main', files: [
                { id: 'main', name: 'main.tex', content: `% NeurIPS uses the neurips_2023.sty package. The compile server fetches it
...
\\end{document}
` },
                { id: 'refs', name: 'refs.bib', content: `@article{lecun2015deep,
  title={Deep learning},
...
` }
    ...
    },
    {
        ...
        make: () => ({ name: 'IEEE Submission', compiler: 'pdflatex', mainFile: 'main', files: [
                { id: 'main', name: 'main.tex', content: `\\documentclass[journal]{IEEEtran}
\\usepackage{amsmath, amssymb}
\\usepackage{graphicx}
\\usepackage{cite}
...
` }
    ...
    },
    {
        ...
        make: () => ({ name: 'Slides', compiler: 'pdflatex', mainFile: 'main', files: [
                { id: 'main', name: 'main.tex', content: `\\documentclass{beamer}
\\usetheme{metropolis}
\\usepackage{amsmath}
...
function FileTree({ project, activeId, onOpen, onAdd, onRename, onDelete, onSetMain, onUpload }) {
    ...
    return (React.createElement("div", { className: "filetree" },
        React.createElement("div", { className: "filetree-actions" },
            React.createElement("button", { className: "ft-act", title: "New file", onClick: onAdd }, "+ File"),
            React.createElement("button", { className: "ft-act", title: "Upload image", onClick: () => { var _a; return (_a = fileInput.current) === null || _a === void 0 ? void 0 : _a.click(); } }, "\u2191 Img"),
            React.createElement("input", { ref: fileInput, type: "file", accept: "image/*", style: { display: 'none' }, onChange: e => { if (e.target.files[0])
                    onUpload(e.target.files[0]); e.target.value = ''; } })),
        React.createElement("div", { className: "filetree-list" }, project.files.map(f => {
            const kind = RC.fileKind(f.name);
            const isMain = f.id === project.mainFile;
            return (React.createElement("div", { key: f.id, className: `ft-item${f.id === activeId ? ' active' : ''}`, onClick: () => onOpen(f.id) },
                React.createElement("span", { className: `ft-icon ft-icon-${kind}` }, KIND_ICON[kind] || KIND_ICON.default),
                React.createElement("span", { className: "ft-name" }, f.name),
                isMain && React.createElement("span", { className: "ft-badge", title: "Main file" }, "main"),
                React.createElement("button", { className: "ft-menu-btn", title: "Options", onClick: e => { e.stopPropagation(); setMenu(menu === f.id ? null : f.id); } }, "\u22EF"),
                menu === f.id && (React.createElement("div", { className: "ft-menu", onClick: e => e.stopPropagation() },
                    !isMain && kind === 'tex' && React.createElement("button", { onClick: () => { onSetMain(f.id); setMenu(null); } }, "Set as main"),
                    React.createElement("button", { onClick: () => { onRename(f.id); setMenu(null); } }, "Rename"),
                    project.files.length > 1 && React.createElement("button", { className: "danger", onClick: () => { onDelete(f.id); setMenu(null); } }, "Delete")))));
        }))));
}
...
function PdfViewer({ pdfUrl, compiling, progress, onDownload }) {
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            if (!doc) {
                ...
                try {
                    doc = await window.pdfjsLib.getDocument(pdfUrl).promise;
                    doc._rtlUrl = pdfUrl;
                    docRef.current = doc;
                }
                catch (e) {
                    if (!cancelled)
                        setRenderErr(e.message || 'Failed to load PDF');
                    return;
                }
            }
            if (cancelled)
    ...
    if (compiling)
        return (React.createElement("div", { className: "pdf-empty" },
            React.createElement("div", { className: "pdf-spinner" }),
            React.createElement("div", { className: "pdf-empty-text" }, "Compiling with TeX Live\u2026"),
            React.createElement("div", { className: "pdf-empty-sub" }, progress && progress.endpoint
                ? React.createElement(React.Fragment, null,
                    progress.endpoint,
                    progress.attempt > 1 ? ` · retry ${progress.attempt}` : '',
                    " \u2014 full engine (TikZ, biblatex, custom classes)")
                : 'Full engine — TikZ, biblatex, custom classes')));
    if (!pdfUrl)
        return (React.createElement("div", { className: "pdf-empty" },
            React.createElement("div", { className: "pdf-empty-icon" }, "\u2318\u21B5"),
            React.createElement("div", { className: "pdf-empty-text" }, "No PDF yet"),
            React.createElement("div", { className: "pdf-empty-sub" },
                "Hit ",
                React.createElement("b", null, "Compile"),
                " (or \u2318/Ctrl+Enter) to render a real, submission-grade PDF.")));
    return (React.createElement("div", { className: "pdf-view" },
        React.createElement("div", { className: "pdf-toolbar" },
            React.createElement("span", { className: "pdf-pages" },
                pageCount,
                " page",
                pageCount !== 1 ? 's' : ''),
            React.createElement("div", { className: "pdf-zoom" },
                React.createElement("button", { onClick: () => setScale(s => Math.max(0.5, +(s - 0.15).toFixed(2))), title: "Zoom out" }, "\u2212"),
                React.createElement("span", { className: "pdf-zoom-val" },
                    Math.round(scale * 100),
                    "%"),
                React.createElement("button", { onClick: () => setScale(s => Math.min(3, +(s + 0.15).toFixed(2))), title: "Zoom in" }, "+")),
            React.createElement("div", { className: "pdf-tb-spacer" }),
            onDownload && React.createElement("button", { className: "pdf-tb-btn", onClick: onDownload }, "\u2193 PDF"),
            React.createElement("a", { className: "pdf-tb-btn", href: pdfUrl, target: "_blank", rel: "noopener" }, "\u2922 Open")),
        renderErr
            ? React.createElement("div", { className: "pdf-empty" },
                React.createElement("div", { className: "pdf-empty-text" }, "Couldn\u2019t render inline"),
                React.createElement("div", { className: "pdf-empty-sub" },
                    renderErr,
                    ". The PDF still compiled \u2014 use ",
                    React.createElement("b", null, "Open"),
                    " or ",
                    React.createElement("b", null, "\u2193 PDF"),
                    "."))
            : React.createElement("div", { className: "pdf-canvas-scroll", ref: containerRef })));
}
...
function LogPanel({ result, diags, onClose, onJump }) {
    ...
    return (React.createElement("div", { className: "log-panel" },
        React.createElement("div", { className: "log-tabs" },
            React.createElement("button", { className: `log-tab${tab === 'problems' ? ' active' : ''}`, onClick: () => setTab('problems') },
                "Problems ",
                errors.length > 0 && React.createElement("span", { className: "badge-err" }, errors.length),
                warnings.length > 0 && React.createElement("span", { className: "badge-warn" }, warnings.length)),
            React.createElement("button", { className: `log-tab${tab === 'raw' ? ' active' : ''}`, onClick: () => setTab('raw') }, "Raw log"),
            React.createElement("div", { className: "log-spacer" }),
            result && React.createElement("span", { className: "log-status" },
                result.ok ? '✓ compiled' : '✕ failed',
                " \u00B7 ",
                Math.round(result.ms),
                "ms"),
            React.createElement("button", { className: "log-close", onClick: onClose }, "\u2715")),
        tab === 'problems' ? (React.createElement("div", { className: "log-body" },
            diags.length === 0 && React.createElement("div", { className: "log-clean" }, result && result.ok ? 'No problems. Clean compile. ✓' : 'No structured diagnostics — see Raw log.'),
            diags.map((d, i) => (React.createElement("button", { key: i, className: `diag diag-${d.type}`, onClick: () => d.line && onJump(d.line) },
                React.createElement("span", { className: "diag-icon" }, d.type === 'error' ? '✕' : '⚠'),
                d.line && React.createElement("span", { className: "diag-line" },
                    "L",
                    d.line),
                React.createElement("span", { className: "diag-msg" }, d.message)))))) : (React.createElement("pre", { className: "log-raw" }, result ? result.log || '(no log returned)' : '(no compile yet)'))));
}
// ── APP ───────────────────────────────────────────────────────────────────────
function App() {
    ...
    const [tweaks, setTweaks] = useState(() => {
        try {
            return { ...TWEAK_DEFAULTS, ...JSON.parse(localStorage.getItem('rtl-tweaks') || '{}') };
        }
        catch (_a) {
    ...
    useEffect(() => {
        const h = e => {
            var _a, _b;
            if (((_a = e.data) === null || _a === void 0 ? void 0 : _a.type) === '__activate_edit_mode')
                setShowTweaks(true);
            if (((_b = e.data) === null || _b === void 0 ? void 0 : _b.type) === '__deactivate_edit_mode')
                setShowTweaks(false);
        };
    ...
    useEffect(() => {
        loadCachedPdf().then(blob => { if (blob)
    ...
    const docMacros = useMemo(() => {
        ...
        project.files.forEach(f => {
            if (window.RTLCompile.fileKind(f.name) !== 'tex')
                return;
            const re = /\\(?:re)?newcommand\{\\([a-zA-Z]+)\}(?:\[\d+\])?\{((?:[^{}]|\{[^{}]*\})*)\}/g;
            let x;
            while ((x = re.exec(f.content)))
                m['\\' + x[1]] = x[2];
        });
        return m;
    }, [project.files]);
    ...
    useEffect(() => {
        ...
        if (!ed || !ed.setDiagnostics)
            return;
        if (activeId === project.mainFile && diags.length)
    ...
    const deleteFile = id => {
        ...
        setProject(p => {
            ...
            const mainFile = p.mainFile === id ? (_a = files[0]) === null || _a === void 0 ? void 0 : _a.id : p.mainFile;
            return { ...p, files, mainFile };
        });
        if (activeId === id)
            setActiveId((_a = project.files.find(x => x.id !== id)) === null || _a === void 0 ? void 0 : _a.id);
    };
    ...
    const compile = useCallback(async () => {
        ...
        const result = await RC.compileProject(project, {
            signal: ctrl.signal,
            onProgress: p => { if (myToken === compileTokenRef.current)
                setCompileProgress(p); },
        });
        // Ignore results from a superseded/canceled run
        if (myToken !== compileTokenRef.current)
            return;
        if (result.canceled) {
            return;
        } // a newer compile took over
        setCompileResult(result);
        const d = RC.parseCompileLog(result.log);
        setDiags(d);
        if (result.ok) {
            setPdfUrl(prev => { if (prev)
                URL.revokeObjectURL(prev); return result.pdfUrl; });
    ...
    const downloadPdf = () => {
        if (!(compileResult === null || compileResult === void 0 ? void 0 : compileResult.pdfBlob))
            return;
    ...
    const handleInsert = useCallback(ins => { var _a; return (_a = editorRef.current) === null || _a === void 0 ? void 0 : _a.insert(ins); }, []);
    ...
    const jumpToLine = useCallback(line => {
        ...
        setTimeout(() => { var _a; return (_a = editorRef.current) === null || _a === void 0 ? void 0 : _a.goToLine(line - 1); }, 50);
    }, [project.mainFile]);
    // Click-to-source from the Live preview (markers map to MAIN file lines)
    const jumpToSource = useCallback(line => {
        if (activeId !== project.mainFile)
            setActiveId(project.mainFile);
        setTimeout(() => { var _a; return (_a = editorRef.current) === null || _a === void 0 ? void 0 : _a.goToLine(line); }, activeId !== project.mainFile ? 60 : 0);
    }, [activeId, project.mainFile]);
    // New project from template
    const newFromTemplate = useCallback(tpl => {
        if (!confirm(`Start a new "${tpl.name}" project? This replaces the current files.`))
            return;
        const proj = tpl.make();
        setProject(proj);
    ...
    return (React.createElement("div", { className: "app", onDragOver: e => { e.preventDefault(); if (!dragOver)
            setDragOver(true); }, onDragLeave: e => { if (e.target === e.currentTarget)
            setDragOver(false); }, onDrop: onDrop },
        React.createElement("input", { ref: importInputRef, type: "file", multiple: true, accept: ".zip,.tex,.bib,.sty,.cls,.txt,.png,.jpg,.jpeg,.pdf", style: { display: 'none' }, onChange: e => { var _a; if ((_a = e.target.files) === null || _a === void 0 ? void 0 : _a.length)
                importFiles(e.target.files); e.target.value = ''; } }),
        dragOver && React.createElement("div", { className: "drop-overlay" },
            React.createElement("div", { className: "drop-card" }, "Drop .zip / .tex / .bib / images to import")),
        React.createElement("div", { className: "topbar" },
            React.createElement("div", { className: "topbar-logo" },
                React.createElement("div", { className: "logo-badge" }, "\u2112"),
                React.createElement("span", null, "Real Time LaTeX")),
            React.createElement("div", { className: "topbar-sep" }),
            React.createElement("button", { className: "icon-toggle", title: "Toggle sidebar", onClick: () => setShowSidebar(v => !v) }, "\u2630"),
            React.createElement("input", { className: "filename-input", value: project.name, spellCheck: false, onChange: e => setProject(p => ({ ...p, name: e.target.value })) }),
            React.createElement("div", { className: "spacer" }),
            React.createElement("span", { className: "stats-label" },
                stats.words,
                " words \u00B7 ",
                stats.eqs,
                " eq \u00B7 ",
                ...
                " files"),
            React.createElement("div", { className: `status-dot ${status}`, title: "Live preview status" }),
            React.createElement("div", { className: "topbar-sep" }),
            React.createElement("select", { className: "compiler-select", value: project.compiler, onChange: e => setProject(p => ({ ...p, compiler: e.target.value })), title: "TeX engine" }, RC.COMPILERS.map(c => React.createElement("option", { key: c, value: c }, c))),
            compiling
                ? React.createElement("button", { className: "compile-btn compiling", onClick: cancelCompile, title: "Cancel compile" },
                    React.createElement("span", { className: "btn-spinner" }),
                    "Cancel")
                : React.createElement("button", { className: "compile-btn", onClick: () => compile() }, "\u25B8 Compile"),
            React.createElement("button", { className: `auto-toggle${autoCompile ? ' on' : ''}`, title: "Auto-compile ~2.5s after you stop typing", onClick: () => setAutoCompile(v => !v) }, "\u27F3 Auto"),
            React.createElement("div", { className: "download-wrap" },
                React.createElement("button", { className: "tbtn tbtn-ghost", onClick: () => setShowTemplates(v => !v), title: "New from template" }, "+ New \u25BE"),
                showTemplates && (React.createElement("div", { className: "download-menu tpl-menu", onMouseLeave: () => setShowTemplates(false) },
                    React.createElement("div", { className: "tpl-menu-head" }, "Start from template"),
                    TEMPLATES.map(t => (React.createElement("button", { key: t.id, className: "tpl-item", onClick: () => newFromTemplate(t) },
                        React.createElement("span", { className: "tpl-name" }, t.name),
                        React.createElement("span", { className: "tpl-desc" }, t.desc)))),
                    React.createElement("div", { className: "tpl-sep" }),
                    React.createElement("button", { className: "tpl-import", onClick: () => { var _a; (_a = importInputRef.current) === null || _a === void 0 ? void 0 : _a.click(); setShowTemplates(false); } }, "\u2191 Import .zip / .tex / .bib\u2026")))),
            React.createElement("div", { className: "download-wrap" }, showDownload && (React.createElement("div", { className: "download-menu", onMouseLeave: () => setShowDownload(false) },
                React.createElement("button", { onClick: () => { downloadActiveTex(); setShowDownload(false); } }, "Current file (.tex)"),
                React.createElement("button", { disabled: !(compileResult === null || compileResult === void 0 ? void 0 : compileResult.pdfBlob), onClick: () => { downloadPdf(); setShowDownload(false); } }, "Compiled PDF"),
                React.createElement("button", { onClick: () => { downloadZip(); setShowDownload(false); } }, "Whole project (.zip)")))),
            React.createElement("button", { className: "tbtn tbtn-ghost", onClick: () => setShowTweaks(v => !v), title: "Tweaks" }, "\u2699")),
        React.createElement("div", { className: "main" },
            showSidebar && (React.createElement("div", { className: "sidebar" },
                React.createElement("div", { className: "sidebar-tabs" },
                    React.createElement("button", { className: `sb-tab${sidebarTab === 'files' ? ' active' : ''}`, onClick: () => setSidebarTab('files') }, "Files"),
                    React.createElement("button", { className: `sb-tab${sidebarTab === 'outline' ? ' active' : ''}`, onClick: () => setSidebarTab('outline') }, "Outline")),
                sidebarTab === 'files'
                    ? React.createElement(FileTree, { project: project, activeId: activeId, onOpen: setActiveId, onAdd: addFile, onRename: renameFile, onDelete: deleteFile, onSetMain: setMain, onUpload: uploadImage })
                    : React.createElement(OutlinePanel, { outline: outline, editorRef: editorRef }))),
            React.createElement("div", { className: "editor-pane", style: { width: splitPct + '%' } },
                React.createElement(Toolbar, { onInsert: handleInsert, showSymbols: showSymbols, setShowSymbols: setShowSymbols }),
                React.createElement("div", { className: "active-file-bar" },
                    React.createElement("span", { className: "afb-icon" }, KIND_ICON[RC.fileKind(activeFile.name)] || '·'),
                    React.createElement("span", { className: "afb-name" }, activeFile.name),
                    activeFile.id === project.mainFile && React.createElement("span", { className: "afb-main" }, "main")),
                activeFile.data
                    ? React.createElement("div", { className: "image-preview" },
                        React.createElement("img", { src: `data:${RC.mimeFor(activeFile.name)};base64,${activeFile.data}`, alt: activeFile.name }),
                        React.createElement("div", { className: "image-note" },
                            "Binary asset \u00B7 referenced from .tex via ",
                            React.createElement("code", null, "\\includegraphics")))
                    : React.createElement(Editor, { ref: editorRef, value: activeFile.content, onChange: updateActiveContent, onFindOpen: () => setShowFind(true), onCompile: compile }),
                showFind && React.createElement(FindBar, { editorRef: editorRef, onClose: () => setShowFind(false) }),
                showSymbols && React.createElement(SymbolPalette, { onInsert: handleInsert })),
            React.createElement("div", { className: "pane-divider", onMouseDown: startDrag }),
            React.createElement("div", { className: "preview-pane", style: { flex: 1 } },
                React.createElement("div", { className: "preview-header" },
                    React.createElement("div", { className: "preview-modes" },
                        React.createElement("button", { className: `pmode${previewMode === 'live' ? ' active' : ''}`, onClick: () => setPreviewMode('live') }, "Live"),
                        React.createElement("button", { className: `pmode${previewMode === 'pdf' ? ' active' : ''}`, onClick: () => setPreviewMode('pdf') },
                            "PDF ",
                            pdfUrl && React.createElement("span", { className: "pmode-dot" }))),
                    React.createElement("div", { className: "preview-header-actions" },
                        previewMode === 'live' &&
                            React.createElement("button", { className: `edit-toggle${editMode ? ' on' : ''}`, title: "Edit the document directly in the preview", onClick: () => { setEditMode(v => !v); setMathEditor(null); } }, editMode ? '✓ Editing' : '✎ Edit'),
                        previewMode === 'live' && editMode &&
                            React.createElement("button", { className: "edit-toggle", title: "Insert a new equation after the current block", onClick: insertEquation }, "+ Eq"),
                        previewMode === 'pdf' && (compileResult || diags.length > 0) &&
                            React.createElement("button", { className: "prev-hbtn", onClick: () => setShowLog(v => !v) },
                                "Log ",
                                errCount > 0 && React.createElement("span", { className: "badge-err" }, errCount),
                                warnCount > 0 && React.createElement("span", { className: "badge-warn" }, warnCount)),
                        previewMode === 'pdf' && pdfUrl && React.createElement("button", { className: "prev-hbtn", onClick: downloadPdf }, "\u2193 PDF"),
                        previewMode === 'live' && React.createElement("button", { className: "prev-hbtn", onClick: () => window.print() }, "Print"))),
                React.createElement("div", { className: "preview-body" },
                    previewMode === 'live'
                        ? React.createElement(LivePreview, { html: html, tweaks: tweaks, onJumpToSource: jumpToSource, editMode: editMode, onEditMath: openMathEditor, onBlockFocus: setFocusedLine, onProseCommit: commitProse })
                        : React.createElement(PdfViewer, { pdfUrl: pdfUrl, compiling: compiling, progress: compileProgress, onDownload: downloadPdf }),
                    previewMode === 'pdf' && showLog &&
                        React.createElement(LogPanel, { result: compileResult, diags: diags, onClose: () => setShowLog(false), onJump: jumpToLine })))),
        showTweaks && React.createElement(TweaksPanel, { tweaks: tweaks, setTweaks: setTweaks, onClose: () => setShowTweaks(false) }),
        mathEditor && React.createElement(MathEditorPopover, { initialLatex: mathEditor.isNew ? '' : (mathEditor.kind && /equation|align|gather|multline|eqnarray/.test(mathEditor.kind)
                ? (mathEditor.raw.replace(/^\\begin\{[^}]+\}/, '').replace(/\\end\{[^}]+\}$/, '').replace(/\\label\{[^}]+\}/, '').trim())
                : mathEditor.raw.replace(/^\$\$?|\$\$?$|^\\\[|\\\]$|^\\\(|\\\)$/g, '').trim()), kind: mathEditor.kind, rect: mathEditor.rect, macros: docMacros, isNew: mathEditor.isNew, onSave: saveMathEditor, onCancel: () => setMathEditor(null) })));
}
