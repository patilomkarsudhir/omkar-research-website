  ...
  while (i < n) {
    ...
    if (ch === '\\') {
      if (text[i+1] === '[') {
        ...
        if (end >= 0) { spans.push({start:i, end:end+2, kind:'bdisplay', raw:text.slice(i,end+2), inner:text.slice(i+2,end)}); i = end+2; continue; }
      }
      if (text[i+1] === '(') {
        const end = text.indexOf('\\)', i+2);
        if (end >= 0) { spans.push({start:i, end:end+2, kind:'pinline', raw:text.slice(i,end+2), inner:text.slice(i+2,end)}); i = end+2; continue; }
      }
      ...
      if (bm) {
        ...
        if (end >= 0) { const e = end+endTok.length; spans.push({start:i, end:e, kind:env, raw:text.slice(i,e), inner:text.slice(i+bm[0].length, end)}); i = e; continue; }
      }
    ...
    if (ch === '$') {
      if (text[i+1] === '$') {
        const end = text.indexOf('$$', i+2);
        if (end >= 0) { spans.push({start:i, end:end+2, kind:'ddisplay', raw:text.slice(i,end+2), inner:text.slice(i+2,end)}); i = end+2; continue; }
      } else {
        let j = i+1;
        while (j < n && text[j] !== '\n' && !(text[j] === '$' && text[j-1] !== '\\')) j++;
        if (j < n && text[j] === '$') { spans.push({start:i, end:j+1, kind:'dinline', raw:text.slice(i,j+1), inner:text.slice(i+1,j)}); i = j+1; continue; }
      }
    }
    i++;
  }
...
function collectMathSpans(files, mainId) {
  const byId = {}; files.forEach(f => byId[f.id] = f);
  const byName = {}; files.forEach(f => { byName[f.name.replace(/\.tex$/,'')] = f; });
  const out = [];
  const seen = new Set();
  function walk(file, depth) {
    if (!file || depth > 6 || seen.has(file.id)) return;
    seen.add(file.id);
    const text = file.content;
    // for the main file, only consider the document body (parser strips preamble)
    let lo = 0, hi = text.length;
    const dm = text.match(/\\begin\{document\}/);
    if (dm) { lo = dm.index + dm[0].length; const de = text.indexOf('\\end{document}', lo); if (de >= 0) hi = de; }
    ...
    for (const e of events) { if (e.span) out.push(e.span); else walk(byName[e.input.name], depth+1); }
    seen.delete(file.id);
  }
  walk(byId[mainId], 0);
  return out;
...
function proseToLatex(node) {
  ...
  node.childNodes.forEach(n => {
    if (n.nodeType === 3) { out += escapeProseText(n.textContent); return; }
    if (n.nodeType !== 1) return;
    const el = n, tag = el.tagName.toLowerCase();
    if (el.classList.contains('math-edit')) { out += el.getAttribute('data-mathraw') || ''; return; }
    if (el.classList.contains('sec-num')) return; // skip auto numbers
    if (el.classList.contains('citation')) {
      const keys = [...el.querySelectorAll('a')].map(a => (a.getAttribute('href')||'').replace('#bib-','')).filter(Boolean);
      out += keys.length ? `\\cite{${keys.join(',')}}` : ''; return;
    }
    if (el.classList.contains('ref-link')) {
      const key = (el.getAttribute('href')||'').replace('#label-','');
      out += /^\(.*\)$/.test(el.textContent.trim()) ? `\\eqref{${key}}` : `\\ref{${key}}`; return;
    }
    if (tag === 'strong' || tag === 'b') out += `\\textbf{${proseToLatex(el)}}`;
    else if (tag === 'em' || tag === 'i') out += `\\emph{${proseToLatex(el)}}`;
    else if (tag === 'code') out += `\\texttt{${proseToLatex(el)}}`;
    else if (tag === 'u') out += `\\underline{${proseToLatex(el)}}`;
    else if (tag === 'a') out += `\\href{${el.getAttribute('href')||''}}{${proseToLatex(el)}}`;
    else if (tag === 'br') out += '\\\\\n';
    else out += proseToLatex(el);
  });
  return out;
}
...
function MathEditorPopover({ initialLatex, kind, rect, onSave, onCancel, isNew, macros }) {
  ...
  useEffect(() => {
    if (rawMode || !window.MathQuill || !mqRef.current) return;
    ...
    try { field.latex(initialLatex || ''); } catch {}
    setTimeout(() => field.focus(), 30);
    return () => { try { mqRef.current && (mqRef.current.innerHTML=''); } catch {} };
  }, [rawMode]);
  ...
  useEffect(() => {
    ...
    } catch (e) { setRawPreview(`<span style="color:#c00">${e.message}</span>`); }
  }, [rawLatex, rawMode]);
  ...
  const applyPalette = (item) => {
    ...
    if (item.cmd) field.cmd(item.cmd);
    else field.write(item.write);
    field.focus();
  };
  ...
  return (
    <>
      <div className="mq-backdrop" onMouseDown={onCancel} />
      <div className="mq-popover" style={{ top, left }} onMouseDown={e=>e.stopPropagation()}>
        <div className="mq-head">
          <span className="mq-title">{isNew ? 'New equation' : 'Edit math'} · <span className="mq-kind">{kind}</span></span>
          <button className="mq-mode" onClick={()=>{
            if (!rawMode && fieldRef.current) setRawLatex(fieldRef.current.latex());
            setRawMode(v=>!v);
          }}>{rawMode ? 'Visual' : '⟨⟩ LaTeX'}</button>
        </div>

        {rawMode ? (
          <div className="mq-raw">
            <textarea ref={taRef} className="mq-textarea" value={rawLatex} spellCheck={false}
              onChange={e=>setRawLatex(e.target.value)} autoFocus
              placeholder="Type LaTeX — multi-line align/matrix supported here" />
            <div className="mq-rawpreview" dangerouslySetInnerHTML={{__html: rawPreview}} />
          </div>
        ) : (
          <div className="mq-field-wrap"><span ref={mqRef} className="mq-field" /></div>
        )}

        <div className="mq-palette">
          {MQ_PALETTE.map((it,i)=>(
            <button key={i} className="mq-pal-btn" title={it.title}
              onMouseDown={e=>{e.preventDefault(); applyPalette(it);}}>{it.label}</button>
          ))}
        </div>

        <div className="mq-actions">
          <span className="mq-hint">Patches only this equation in your source</span>
          <div className="mq-action-btns">
            <button className="mq-cancel" onClick={onCancel}>Cancel</button>
            <button className="mq-save" onClick={handleSave}>Save</button>
          </div>
        </div>
      </div>
    </>
