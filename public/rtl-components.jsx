...
function extractOutline(latex) {
  const result = []; const lines = latex.split('\n');
  ...
  lines.forEach((line, lineIdx) => {
    ...
    if (!star) {
      if (env==='chapter'||env==='section'){s++;sub=0;subsub=0;}
      else if (env==='subsection'){sub++;subsub=0;}
      else if (env==='subsubsection') subsub++;
    }
...
const Editor = forwardRef(function Editor({ value, onChange, onFindOpen, onCompile }, apiRef) {
  ...
  useEffect(() => {
    if (!taRef.current || !window.CodeMirror) return;
    ...
    function latexHint(cm) {
      ...
      const list = AC_COMMANDS.filter(c => c.startsWith(token)).slice(0,30).map(c => {
        ...
        return { text:c, displayText:c, hint:(cm,data,comp)=>{
          ...
          if (braceIdx>-1) cm.setCursor({ line:data.from.line, ch:data.from.ch+braceIdx+1 });
        }};
      });
      return list.length ? { list, from, to:cursor } : null;
    }

    cm.on('change', () => { if (!suppressRef.current) onChangeRef.current(cm.getValue()); });
    cmRef.current = cm;
    return () => { try { cm.toTextArea(); } catch(e){} };
  }, []);
  ...
  useEffect(() => {
    ...
    const cur = cm.getCursor(); const scroll = cm.getScrollInfo();
    cm.setValue(value || '');
    cm.setCursor(cur); cm.scrollTo(scroll.left, scroll.top);
  ...
  useImperativeHandle(apiRef, () => ({
    insert(text) {
      ...
      if (pipeIdx>-1) cm.setCursor({ line:cursor.line, ch:cursor.ch+pipeIdx });
      cm.focus();
    },
    goToLine(lineNum) {
      const cm = cmRef.current; if (!cm) return;
    ...
    find(query) {
      const cm = cmRef.current; if (!cm || !query) { this.clearFind(); return 0; }
      marksRef.current.forEach(m=>m.clear()); marksRef.current=[];
      const cur = cm.getSearchCursor(query, null, { caseFold:true });
      let count=0, first=null;
      while (cur.findNext()) { count++; if(!first) first=cur.from();
        marksRef.current.push(cm.markText(cur.from(), cur.to(), { className:'find-highlight' })); }
      if (first) { cm.setCursor(first);
        const t = cm.charCoords(first,'local').top;
        cm.getScrollerElement().scrollTop = Math.max(0, t-80); }
      return count;
    },
    findNext(query, forward=true) {
      const cm = cmRef.current; if (!cm || !query) return;
      ...
      if (found) { cm.setSelection(cur.from(), cur.to());
        const t = cm.charCoords(cur.from(),'local').top;
        cm.getScrollerElement().scrollTop = Math.max(0, t-80); }
      else { const wrap = cm.getSearchCursor(query, forward?{line:0,ch:0}:{line:cm.lastLine(),ch:1e9}, {caseFold:true});
        if (forward?wrap.findNext():wrap.findPrevious()) cm.setSelection(wrap.from(), wrap.to()); }
    },
  ...
  return <div className="cm-wrap"><textarea ref={taRef} defaultValue={value} /></div>;
});
    ...
    <div className="editor-toolbar">
      {TOOLBAR.map((group, gi) => (
        <React.Fragment key={gi}>
          {gi>0 && <div className="ebar-sep" />}
          <span className="ebar-label">{group.label}</span>
          {group.items.map((item, ii) => (
            <button key={ii} className="ebar-btn" title={item.title}
              style={item.style||{}} onClick={()=>onInsert(item.ins)}>{item.t}</button>
          ))}
        </React.Fragment>
      ))}
      <div className="ebar-sep" />
      <button className={`ebar-btn${showSymbols?' active':''}`} title="Symbol palette"
        onClick={()=>setShowSymbols(v=>!v)}>Ω</button>
    </div>
  );
}
  ...
  return (
    <div className="symbol-palette">
      <div className="sym-tabs">
        {Object.keys(SYMBOL_DATA).map(cat => (
          <button key={cat} className={`sym-tab${tab===cat?' active':''}`}
            onClick={()=>setTab(cat)}>{cat}</button>
        ))}
      </div>
      <div className="sym-grid">
        {(SYMBOL_DATA[tab]||[]).map(([disp,latex],i) => (
          <button key={i} className="sym-btn" title={latex}
            onClick={()=>onInsert(latex+' ')}>{disp}</button>
        ))}
      </div>
    </div>
  );
...
function FindBar({ editorRef, onClose }) {
  ...
  const handleChange = e => { const q=e.target.value; setQuery(q); setCount(editorRef.current?.find(q)||0); };
  const handleClose = () => { editorRef.current?.clearFind(); onClose(); };
  return (
    <div className="find-bar">
      <span className="find-icon">⌕</span>
      <input ref={inputRef} className="find-input" placeholder="Find in file…"
        value={query} onChange={handleChange}
        onKeyDown={e => { if(e.key==='Escape') handleClose(); if(e.key==='Enter') editorRef.current?.findNext(query, !e.shiftKey); }} />
      {query && <span className="find-count">{count} match{count!==1?'es':''}</span>}
      <button className="find-nav" title="Previous" onClick={()=>editorRef.current?.findNext(query,false)}>↑</button>
      <button className="find-nav" title="Next" onClick={()=>editorRef.current?.findNext(query,true)}>↓</button>
      <button className="find-close" onClick={handleClose}>✕</button>
    </div>
  );
}
...
function OutlinePanel({ outline, editorRef }) {
  if (!outline.length) return <div className="outline-empty">No sections yet</div>;
  return (
    <div className="outline-list">
      {outline.map((item, i) => (
        <button key={i} className={`outline-item outline-${item.env}`} title={item.title}
          onClick={()=>editorRef.current?.goToLine(item.line)}>
          <span className="outline-num">{item.star?'':item.num}</span>
...
function LivePreview({ html, tweaks, onJumpToSource, editMode, onEditMath, onProseCommit, onBlockFocus }) {
  ...
  const blockKind = (el) => {
    if (el.classList.contains('latex-section')) return 'section';
    if (el.classList.contains('latex-subsection')) return 'subsection';
    if (el.classList.contains('latex-subsubsection')) return 'subsubsection';
    if (el.classList.contains('latex-chapter')) return 'chapter';
  ...
  useEffect(() => {
    ...
    if (editMode) {
      doc.querySelectorAll('[data-src-line]').forEach(el => {
        ...
        if (!kind || el.querySelector('.fn-ref')) return;
  ...
  const handleClick = (e) => {
    ...
    if (el) onJumpToSource(parseInt(el.getAttribute('data-src-line')));
  };

  // Commit prose edits on blur
  const handleBlur = (e) => {
    if (!editMode || !onProseCommit) return;
    ...
    if (isNaN(startLine) || !kind) return;
    const latex = window.RTLEdit.proseToLatex(block).replace(/\s+\n/g,'\n').trim();
    onProseCommit({ startLine, kind, latex });
  };

  const handleFocus = (e) => {
    if (!editMode || !onBlockFocus) return;
    const block = e.target.closest && e.target.closest('.prose-editable');
    if (block) { const l = parseInt(block.getAttribute('data-src-line')); if (!isNaN(l)) onBlockFocus(l); }
  };

  return (
    <div className="preview-scroll">
      <div className={`preview-document${onJumpToSource&&!editMode?' clickable-src':''}${editMode?' edit-mode':''}`}
        ref={docRef}
        onClick={handleClick}
        onBlur={handleBlur}
        onFocus={handleFocus}
        style={{ background:paperBg[tweaks.paperStyle]||paperBg.white,
          color:paperFg[tweaks.paperStyle]||paperFg.white,
          fontFamily:fonts[tweaks.previewFont]||fonts['EB Garamond'],
          fontSize:tweaks.fontSize+'px', lineHeight:tweaks.lineHeight }}
  ...
  return (
    <div className="tweaks-panel">
      ...
      <div className="tw-row">
        <label className="tw-label">Preview Font</label>
        <select className="tw-select" value={tweaks.previewFont} onChange={e=>set('previewFont',e.target.value)}>
          {['EB Garamond','Georgia','Palatino','Times'].map(f=><option key={f}>{f}</option>)}
        </select>
      </div>
      <div className="tw-row">
        <label className="tw-label">Font Size: {tweaks.fontSize}px</label>
        <input type="range" className="tw-slider" min={13} max={24} step={0.5}
          value={tweaks.fontSize} onChange={e=>set('fontSize',parseFloat(e.target.value))} />
      </div>
      <div className="tw-row">
        <label className="tw-label">Line Height: {tweaks.lineHeight}</label>
        <input type="range" className="tw-slider" min={1.3} max={2.2} step={0.05}
          value={tweaks.lineHeight} onChange={e=>set('lineHeight',parseFloat(e.target.value))} />
      </div>
      <div className="tw-row">
        <label className="tw-label">Live Preview Paper</label>
        <select className="tw-select" value={tweaks.paperStyle} onChange={e=>set('paperStyle',e.target.value)}>
          {['white','warm','sepia','dark'].map(s=><option key={s}>{s}</option>)}
        </select>
      </div>
    </div>
  );
